...
!define MUI_PAGE_CUSTOMFUNCTION_LEAVE FinishLeaveWebsiteCheckbox
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
...
Function .onInit
  ...
  ${If} $INSTDIR == "${PLACEHOLDER_INSTALL_DIR}"
    ; Set default install location - nested under ${MANUFACTURER} so it reads
    ; "C:\Program Files\TheNerdyBox\Stream Composer Suite" instead of
    ; installing directly into Program Files' root, for brand recognition
    ; and to group any future TheNerdyBox apps under one parent folder.
    !if "${INSTALLMODE}" == "perMachine"
      ${If} ${RunningX64}
        !if "${ARCH}" == "x64"
